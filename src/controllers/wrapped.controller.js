...
class WrappedController {
    ...
    async getWrappedData(req, res) {
        try {
            ...
            // Step 2: Scrape additional visual data (as fallback/supplement)
            const scrapedProfile = await scraperService.scrapeUserProfile(apiData.relativePath);
            const scrapedScores = await scraperService.scrapeUserScores(apiData.relativePath);

            // Step 3: Process and calculate wrapped statistics
            const wrappedStats = this.calculateWrappedStats(apiData.data);
            ...
            const wrappedData = {
                ...
                scores: {
                    ...
                    level: this.getBuilderScoreLevel(apiData.data.profile?.builderScore?.points)
                },
                ...
                activity: {
                    ...
                    events: {
                        ...
                        timeline: this.processEventsTimeline(apiData.data.events)
                    },
                    scraped: scrapedProfile.success ? scrapedProfile.data : null
                },
                
                credentials: {
                    all: apiData.data.credentials,
                    count: apiData.data.credentials?.length || 0,
                    byCategory: this.groupCredentialsByCategory(apiData.data.credentials)
                },
    ...
    calculateWrappedStats(data) {
        const currentYear = new Date().getFullYear();
        const yearStart = new Date(`${currentYear}-01-01`);
        
        const stats = {
            year: currentYear,
            builderScoreGrowth: this.calculateScoreGrowth(data.events),
            topActivities: this.getTopActivities(data.events),
            milestones: this.extractMilestones(data.events, data.dataPoints),
            monthlyActivity: this.getMonthlyActivityBreakdown(data.events),
            mostActiveMonth: null,
            credentialsEarned: this.getCredentialsThisYear(data.credentials, yearStart),
            projectsLaunched: this.getProjectsThisYear(data.projects, yearStart),
            connectionsGrown: this.calculateConnectionGrowth(data.events)
        };
    ...
    extractMilestones(events, dataPoints) {
        const milestones = [];
    ...
    getMonthlyActivityBreakdown(events) {
    ...
    getCredentialsThisYear(credentials, yearStart) {
    ...
    getProjectsThisYear(projects, yearStart) {
        if (!projects) return [];
    ...
    calculateConnectionGrowth(events) {
    ...
    groupCredentialsByCategory(credentials) {
